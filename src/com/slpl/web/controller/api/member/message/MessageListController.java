package com.slpl.web.controller.api.member.message;
...
@WebServlet("/api/member/message/list")
public class MessageListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(field != null && query != null) {   //검색임
			allCount = service.getViewList(isSender, memberId, field, query).size();   //검색 결과에 대한 전체 count 갖고오기
			list = service.getViewList(isSender, memberId, page, pageItemCount, field, query);
		} else {
			allCount = service.getViewList(isSender, memberId).size();   //전체 목록 수
			list = service.getViewList(isSender, memberId, page, pageItemCount);
		}
