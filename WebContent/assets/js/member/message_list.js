...
window.addEventListener("load", function() {
	...
	var memberId = container.querySelector(".memberId");
	...
	function makeURL() {
		var url = "/api/member/message/list";
		//검색어가 있으면 같이 넘겨주기
		if(fieldInput.value != "" && queryInput.value != "") {
			url += "?memberId="+memberId.value+"&field="+fieldInput.value+"&query="+queryInput.value+"&page=";
		} else {
			url += "?memberId="+memberId.value+"&page=";
		}
		return url;
	}
